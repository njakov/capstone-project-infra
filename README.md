...
2.  **Compute (`modules/gke`, `modules/runner`, `modules/arc`):**
    ...
    * **ARC:** ephemeral app runners (`petclinic-arc-{env}`) in the same GKE cluster via `modules/arc`, using `github-app-runner-sa-{env}` Workload Identity. That account has `roles/artifactregistry.writer` on `{app}-repo-{env}` only (not project-level) and custom role `arcAppDeploy` (`container.clusters.get`, `container.clusters.getCredentials`, `container.clusters.connect`, `container.namespaces.get`). Kubernetes RBAC is a Role in `petclinic` plus get Services in `ingress-nginx`. `roles/artifactregistry.writer` stays on `terraform-sa-binder-workload` so the next bootstrap can revoke a leftover project-level grant. Cutover steps: [docs/arc-cutover.md](docs/arc-cutover.md).
