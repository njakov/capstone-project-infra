...
| In-cluster isolation | Dedicated namespaces (`arc-systems` / `arc-runners`), tainted `runners` node pool, demo-quality NetworkPolicies, Workload Identity. The `petclinic` namespace sets `pod-security.kubernetes.io/enforce=baseline` and audit/warn `restricted` |
| Build strategy on ARC | **Rootless BuildKit** on Ubuntu runner nodes; no DinD |
| Infra CI path | Single apply path: [`infra-pipeline.yml`](../../.github/workflows/infra-pipeline.yml). The setup job runs on `ubuntu-24.04` and only resolves environment, ref, and runner label. Validate, plan, apply, and destroy use `runs-on: [self-hosted, infra, {env}]`. Before `terraform init` those jobs fail closed unless `environments/{env}/terraform.tfvars` exists and `project_id` parses. The state bucket is `terraform-state-bucket-${project_id}`. Those three names are not read from GitHub. The GitHub Environment supplies `ALLOWED_SOURCE_RANGES` and `GRAFANA_ADMIN_PASSWORD`, and is the deployment-protection boundary |
...
Infra ↔ app VPC peering (env-owned) still connects the two VPCs. Peering exports and imports custom routes (`export_custom_routes` and `import_custom_routes` default to true). Control-plane access for Terraform is the GKE **DNS endpoint**: peering is not transitive, and Regular-channel clusters publish the control plane over Private Service Connect. `allow_external_traffic` makes that name reachable from the infra runner via Private Google Access. Access is Google IAM only (`container.clusters.connect`). Kubernetes ServiceAccount tokens and client certificates stay disabled on that name (`enable_k8s_tokens_via_dns` and `enable_k8s_certs_via_dns` are false); Helm and Terraform send the runner's Google access token. VPC Service Controls are unavailable on this trial account (no organization), so the DNS name has no network allowlist. `enable_private_nodes` and `enable_private_endpoint` stay true, and IP endpoints stay enabled. Helm and the Kubernetes provider use `https://<dns endpoint>` and omit `cluster_ca_certificate` (the DNS name presents a Google-managed certificate; passing the cluster CA makes TLS fail). `master_authorized_networks_config` lists only the app subnet for the IP endpoint. The infra runner subnet is not listed: peering does not advertise the master CIDR, so that allowlist entry never opened a path.
...
| `github-infra-runner-sa-{env}` (GCE) | Day-2 Terraform apply (`infra-pipeline.yml`) | Project: `networkAdmin`, `container.admin`, `cloudsql.admin`, `secretmanager.admin`, `artifactregistry.admin`, `serviceUsageConsumer`; bucket `objectAdmin`. **No** `projectIamAdmin`, **no** project-level `serviceAccountAdmin` / `serviceAccountUser`. Resource-level (bootstrap D6): `infraRunnerWorkloadIdentityAdmin` on the app, app-runner, and external-secrets accounts. The condition is `hasOnly(['roles/iam.workloadIdentityUser'])`, so `setIamPolicy` may only change that role and `getIamPolicy` stays allowed. `serviceAccountUser` on the node SA. |
| `github-app-runner-sa-{env}` (WI → ARC) | App build/deploy only | `roles/artifactregistry.writer` on repository `{app}-repo-{env}` only (not project-level). Custom role `arcAppDeploy` (`container.clusters.get`, `container.clusters.getCredentials`, `container.clusters.connect`, `container.namespaces.get`). Kubernetes Role `app-runner-deploy` in `petclinic` can create, update, and delete Deployments, ReplicaSets, Services, ConfigMaps, ServiceAccounts, Secrets, Pods, Ingresses, SecretProviderClasses, ServiceMonitors, and PrometheusRules, and can get pod logs. A second Role get/lists Services in `ingress-nginx`. Helm writes release Secrets `sh.helm.release.v1.petclinic.vN` in `petclinic`. The chart SecretProviderClass has no `secretObjects`; database files are CSI mounts. **No** `container.developer`, **no** `container.secrets.*`, **no** namespace create/update, **no** binding in `arc-runners`, **no** `projectIamAdmin`, **no** state admin, **no** `networkAdmin`, **no** `secretmanager.admin`. `roles/artifactregistry.writer` stays on `terraform-sa-binder-workload` so bootstrap can revoke a leftover project-level grant. |

**Binder allow-list** (CEL on terraform-sa `projectIamAdmin`): two conditional bindings, because one `hasOnly` list holds at most 10 roles. IAM ORs them. `terraform-sa-binder-workload`: `networkAdmin`, `container.admin`, `cloudsql.admin`, `secretmanager.admin`, `artifactregistry.admin`, `serviceUsageConsumer`, log/metric writers, AR reader/writer. `terraform-sa-binder-scoped`: `container.developer`, `cloudsql.client`, `projects/<project>/roles/arcAppDeploy`. `container.developer` stays on the scoped list so bootstrap can revoke the old app-runner binding. It is not granted to any member. Off-list roles (incl. `owner`, `projectIamAdmin`, SA admin/user) → fix Terraform, **do not widen CEL**. Cleanup of off-list roles is Owner-only.
...
**Negative IAM tests** (teaching demo; `scripts/negative-iam-tests.sh`): terraform-sa cannot bind `roles/owner` or dry-run-create a GKE cluster or a Cloud SQL instance; the runner cannot add `roles/iam.serviceAccountUser` on `terraform-sa`. The script also checks that `github-app-runner-sa-{env}` has no project-level `roles/artifactregistry.writer` and that the node SA has no project-level `roles/artifactregistry.reader`. Expect `PERMISSION_DENIED` or an absent binding.

The tighter demo is still worth adding and is not in the script: the runner cannot bind `roles/iam.serviceAccountKeyAdmin` on the workload service accounts, cannot mint keys on those accounts including `external-secrets-{env}`, and can bind then remove `roles/iam.workloadIdentityUser` for `petclinic/iam-condition-probe`. Those checks match the D6 condition.
...
| Runner cannot rewrite project IAM or hijack terraform-sa | Runner owns app VPC (`networkAdmin`). D6 condition is `hasOnly(['roles/iam.workloadIdentityUser'])`; `getIamPolicy` stays allowed |
...
- The setup job in [`infra-pipeline.yml`](../../.github/workflows/infra-pipeline.yml) runs on `ubuntu-24.04` and does not run Terraform. Validate, plan, apply, and destroy use `runs-on: [self-hosted, infra, {env}]`. `project_id` and `region` come from `environments/{env}/terraform.tfvars`. The shared checkout guard fails before `terraform init` when that file is missing or `project_id` cannot be parsed, and the state bucket is `terraform-state-bucket-${project_id}`. CI does not read `GCP_PROJECT_ID`, `GCP_REGION`, or `TF_STATE_BUCKET` from GitHub. Delete those variables at repository scope after any Environment copies exist, then delete the `dev` and `prod` Environment copies. Those jobs do read `ALLOWED_SOURCE_RANGES` and `GRAFANA_ADMIN_PASSWORD` from the GitHub Environment. Required reviewers cannot be set from the workflow; add them on the `prod` Environment (operator step). Validate, plan, apply, and destroy all attach that Environment, so configured reviewers pause every prod job, including plan. The workflow's own check that the required-reviewer rule exists runs only for prod apply and destroy. Prod plan may be the tip of `main` or an ancestor; prod apply and destroy must be the current tip. Extending that fail-closed check to prod plan and validate is the tighter gate and is not implemented.
...
- Role `app-runner-deploy` lets a compromised ARC runner create, update, and delete Deployments, ReplicaSets, Services, ConfigMaps, ServiceAccounts, Secrets, Pods, Ingresses, SecretProviderClasses, ServiceMonitors, and PrometheusRules in `petclinic`, and read pod logs. It can retarget the SecretProviderClass so the CSI driver writes database files into a Kubernetes Secret, or roll an image that reads `/mnt/secrets`. `ingress-nginx` stays get/list on Services. This is accepted for the demo. The tighter bound, still achievable and not implemented: Terraform owns the ServiceAccount, SecretProviderClass, Ingress, Service, ConfigMap, ServiceMonitor, and PrometheusRule, and the runner Role can only update the app image. Image rollout into that pod would still run as the app identity with the CSI volume. Splitting build and deploy so only an attested image can roll out closes that path; Binary Authorization stays a non-goal.
- The GCE infra VM startup script installs Docker. The socket is group `docker` (`runner` user), mode 660, never `chmod 666`. The VM is Ubuntu 22.04 LTS, 50 GB, CMEK, zone `{region}-b`. Ubuntu 24.04 in the cluster-evidence note is the GKE runner node. Day-2 infra path is Terraform-only on `e2-medium`.
...
- App-runner Kubernetes Role in `petclinic` can mutate Secrets, ServiceAccounts, Ingresses, and SecretProviderClasses, not only Deployments. GCP `arcAppDeploy` still has no `container.secrets.*`
