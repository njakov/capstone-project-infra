...
- The BuildKit sidecar runs as UID 1000 and writes a docker-archive tarball to a shared emptyDir. The runner container stays UID 0 so `run-helper.sh` can start (`RUNNER_ALLOW_RUNASROOT`). Neither container is privileged and neither mounts a Docker socket. The pod sets `hostUsers: false` so Kubernetes allows `procMount: Unmasked`. The sidecar starts `buildkitd` directly; rootlesskit is not used, because a nested `newuidmap` is denied inside that user namespace.
