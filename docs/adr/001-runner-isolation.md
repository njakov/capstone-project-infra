...
| `github-app-runner-sa-{env}` (WI → ARC) | App build/deploy only | `roles/artifactregistry.writer` on repository `{app}-repo-{env}`, plus the project-level writer binding until that repository binding is applied. Custom role `arcAppDeploy` (`container.clusters.get`, `container.clusters.getCredentials`, `container.clusters.connect`, `container.namespaces.get`). Kubernetes RBAC is a Role in `petclinic` plus get/list Services in `ingress-nginx`. **No** `container.developer`, **no** `container.secrets.*`, **no** namespace create/update, **no** binding in `arc-runners`, **no** `projectIamAdmin`, **no** state admin, **no** `networkAdmin`, **no** `secretmanager.admin`. After the env apply creates the repository binding, remove project-level `artifactregistry.writer` from `app_runner_roles` and bootstrap-apply. That role stays on `terraform-sa-binder-workload` so the revoke is allowed. |
